...
if __name__ == '__main__':
    ...
    while (True):
        ...
        if option == 'c':
            articleObject = crawler.crawler(ChromeDriverDir)
            sender.sender(recruitData, articleObject, ChromeDriverDir)
        elif option == 'q':
            print("bye <3")
            break;
        elif option == 'h':
            ...
            print(
                "네이버 뉴스의 기사를 받아와서 자동으로 육군 훈련소 인터넷 편지를 보냅니다. 첫 편지 발송 전 육군 훈련소 홈페이지에서 본인인증을 해야 하며, 이 프로그램의 지시를 따라 진행하시면 됩니다. 본인 인증 후 일정 시간 동안은 추가 인증없이 계속 발송할 수 있습니다.",
                end="\n")
